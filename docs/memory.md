...
3. **Write** - the model closes the loop itself with the `save_memory` tool
   (`{ scope: "global" | "workspace", content: string }`). Saving memory is
   approval-gated because the result is injected into future sessions. The tool
   is available even when no workspace folder is open (workspace scope then
   fails with a clear error). The prompt instructs the model to save user
   preferences, corrections, project facts, and gotchas - and never secrets or
   trivia.
